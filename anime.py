...
function DayCell({ day, isToday, isStart, isEnd, inRange, isWeekend, holiday, hasNote,
                   ...
                   onClick, onMouseEnter, onMouseLeave, isDark }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center",
                    ...
                    boxShadow: (isStart || isEnd) ? `0 0 15px ${accentColor}` : (hov && !isStart && !isEnd ? "0 4px 10px rgba(0,0,0,0.2)" : "none") }}>
        {day}
        {isToday && (
...
export default function WallCalendar() {
  ...
  }, [phase, startDate, year, month]);

  const clearSel = useCallback(() => { setStart(null); setEnd(null); setPhase("start"); setHovered(null); }, []);

  const effectiveEnd = endDate || (phase === "end" && hovered && startDate && hovered >= startDate ? hovered : null);
  ...
  const selLabel = (() => {
    ...
    if (phase === "end") return `From ${startDate.toLocaleDateString("en-US",{month:"short",day:"numeric"})}`;
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:680px){.wc-body{grid-template-columns:1fr}.wc-hero{min-height:210px!important;max-height:280px}}

        .wc-hero{position:relative;overflow:hidden;min-height:570px}
        .wc-hero-bg{position:absolute;inset:0;transition:background .6s cubic-bezier(0.85, 0, 0.15, 1)}
        ...
        .wc-grid{display:grid;grid-template-columns:repeat(7,1fr);
                 transition:opacity .25s ease,transform .35s cubic-bezier(0.25, 1, 0.35, 1);
                 transform-style:preserve-3d;transform-origin:center center}
        .wc-grid.fading{opacity:0; transform:rotateY(${tDir * -50}deg) translateX(${tDir * 250}px) scale(0.6) skewX(${tDir * -15}deg); filter: blur(6px)}
            ...
            <div className="wc-hero-ov">
              ...
              {phase === "end" && startDate && !endDate && (
                <div className="wc-pill">
                  <span className="wc-pulsedot" /> Select Target
                </div>
              )}
              {startDate && endDate && (
            ...
            <div className={`wc-grid${fading ? " fading" : ""}`}>
              {calDays.map((day, i) => {
                ...
                const isEnd   = isSame(d, endDate) ||
                                (phase === "end" && hovered && isSame(d, hovered) && startDate && d >= startDate);
                ...
                return (
                  <DayCell
                    ...
                    colIdx={colIdx} accentColor={ac} lightColor={lgt} effectiveEnd={effectiveEnd}
                    startDate={startDate} onClick={() => handleClick(day)}
                    onMouseEnter={() => phase === "end" && setHovered(new Date(year, month, day))}
            ...
            <div className="wc-notes">
              ...
              <div className="wc-inputrow">
                <textarea
                  ...
                  placeholder={
                    !startDate        ? "Select your dates first..." :
                    phase === "end"   ? "Awaiting target end date..." :
                    `Input mission for ${fmtRange(startDate, endDate) || "target"}...`
                  }
...
with codecs.open(r"C:\Users\admin\Downloads\wallcalendar\calendar\src\WallCalendar.jsx", "w", "utf-8") as f:
    f.write(content)
with codecs.open(r"C:\Users\admin\Downloads\wallcalendar\WallCalendar.jsx", "w", "utf-8") as f:
    f.write(content)
